...
Traffic Signal Control (TSC), a classic control problem applicable to everyone who drives, has exciting potential for reinforcement learning; aiding in obvious societal problems: air pollution, decrease in speed, delays, and opportunity costs \cite{Almeida2022MultiagentRL}.
...
In this paper, we will discuss the TSC problem in terms of the Markov Decision Process (MDP) paradigm; how it is formally described.
...
Multiagent approaches to TSC are Partially Observable Markov Decision Proccesses (POMDPs) with non-stationary dynamics, placing them in one of the most difficult classes of problems for RL to solve \cite{Almeida2022MultiagentRL} \cite{DBLP:journals/corr/abs-2004-04778} \cite{choi1999environment}.
...
In fact, state aliasing is common in POMDPs where the agent finds two states that are different to be the same, resulting in inappropriate actions.
...
In order to change the intersection into a different mode, a mandatory yellow phase \phi preceeds the mode change.
In most simulation scenarios, \phy = 2 seconds.
...
Other papers extend this idea to the sum of all vehicle wait time in the system to encourage cooperation between agents \cite{10.3389/frai.2022.805823}.
Furthermore, other sophisticated reward functions such as total delays imposed by the intersection, traffic pressure, augmented rewards based on intersection efficiency, and rewards based on ontology adherance have been explored by some papers \cite{ault2021reinforcement} \cite{Ghanadbashi2023}.
In this project, the cumulative delay reward function \ref{eq:cumulative_delay} will be used, however, the cumulative delay will be the sum of all vehicles in the system instead of at the intersection level, where \(V_t\) is the set of all vehicles present in the simulation.
...
Additionally, SUMO was chosen for its compatability with the OpenAI PettingZoo API which helps create a more standard environment interface for RL systems \cite{terry2021pettingzoo}.
...
The experience is then used to train a deep nerual network and is randomly shuffled to smooth the training data across many past behaviors \cite{DBLP:journals/corr/MnihKSGAWR13}.
The input to the network is the state information, and the output is a Q-value mapped to each action.
In the past, non-linear Q-value function approximators (such as a deep nerual network) were unsucessful due to correlations in the sequence of states, and small updates to Q significantly changing the policy \cite{hafiz2020deep}.
Experience replay and other DQN methods showed to significantly increase the chances of convergence and improve performance by an order of magnitude \cite{Mnih2015}.
...
An off-policy action selection method known as \eps-greedy will be used, where \eps is the probability the agent will select a random action instead of following the learned policy.
Higher values of \eps encourage exploration, while lower values encourage exploitation.
The value of \eps does not have to be constant throughout the learning process, and choosing an \eps value that decays linearly from 0.99 to 0.01

\section{Performance Metrics}
% - RESCO, injected uncertainty, average vehicle wait time
% - REinforced Signal COntrol or RESCO is a testbed and benchmark environment for comparison between RL-based signal control algorithms
REinforced Signal COntrol (RESCO) is a testbed and benchmark environment to help judge performance of RL-based algorithms for TSC. It comes built-in with the SUMO project and will simulate traffic in the same way it was simulated in other benchmark papers \cite{ault2021reinforcement}. This can be used to compare this project with what has already been done.
The metrics that are relavant to this project include: total wait time for each vehicle, average delay per vehicle, average number of stops, average queue length, and average trip time \cite{Reza2023}.
Studying how the agents respond to injected uncertainty such as an emergency vehicle or increased traffic demands is also an important metric for designing a robust system \cite{DBLP}.
% - total waiting time, average delay per vehicle, average number of stops \cite{Reza2023}

\section{Expected Challenges}
The non-stationary nature of multi-agent RL problems is known to cause divergance problems due to the constantly changing dynamics.
This can be somewhat mitigated by applying a scenario that is consistent accross training, but the dynamics will still be changing due to the other agents' learning \cite{DBLP:journals/corr/abs-2004-04778}.
Introducing context switches by changing the scenario, for example, making vehicles flow in waves to simulate rush hour is critical to designing a robust traffic system, but will be out of scope of the initial research goals.
