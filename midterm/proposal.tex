...
The Simulated Urban MObility (SUMO) environment will be used for experimentation because it has been evaluated and calibrated by the general transportation community \cite{ault2021reinforcement} and is widely used in the literature.
...
IDQN is an off-policy, model-free RL method which uses a deep neural network (Q-network) to map states to actions.
...
A convolutional layer can be added to the Q-network showing an increase in performance if the state can be represented as an image.
The idea behind this is to create an image which a 2x2 kernal can convolve gathering more meaningful state information if nearby pixels map to state information belonging to the same road \cite{ault2020learning}.
...
Huber loss is used as a target for the Q-network to minimize using the Adam optimizer which is a variant of the stochastic gradiant desent algorithm.
...
% - learning is fast for quadratic loss to to convex nature
% - huber loss combines resistant to outliers and fast learning
% huber loss is a piecewise function combining quadratic loss and absolute loss in a way that is differentiable
...
For brevity, the fixed time results applied to the 4x4 grid scenario has been left out of this paper.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{../sumo-rl/Data_dqn_multi_1/avg_delay.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{../sumo-rl/Data_dqn_multi_1/total_stopped.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{multi_loss.png}
